...
| [mainpage-restructure](./mainpage-restructure/) | 2026-07-20 | 100% | 0 | 1 세션 | 홈 IA·동선 재배치 — 자료원이 2026-07-14 `homepage-learning-hub` 기준 4개에서 **11개**(교과서 8종 포함)로 성장했으나 홈이 못 따라간 노후화 해소. `PlatformHero` 하드코딩 "4개 자료원"을 `SOURCES.length` 파생값으로 교정, **`SourcePicker`+`LearningPathSection`을 `PerspectiveCatalog` 1개로 통합**해 accent축(원리·위험·안전·직무) 군집화 — 관점당 자료원 1개 제약 해소로 **교과서 8종 전부 홈 노출**(이전엔 1종만). **`RoleQuickEntry` 신규**(목표 pill 4개 → 관점 군집 해시 앵커, 개인화·분기 페이지 없음). `#sources` 앵커(Header·Hero CTA 참조) 승계로 링크 무수정. sweep이 설계 범위 밖 동일 하드코딩 3곳(`seo.ts`·`Footer.tsx`·`page.tsx` 푸터)을 추가 적발해 함께 교정(FR-1 확장 적용, gap-detector가 "정당"으로 판정). gap-detector 독립 검증 **Match Rate 98%→100%**(G-1: 설계 문언 `<a>` vs 구현 `<Link>` — 기능 동일, 설계 문언을 코드에 정렬해 해소). 게이트 전부 통과(typecheck·lint·build exit 0, SSG 페이지 수 유지, 홈 렌더 실측 — 4클러스터 앵커·11자료원·8교과서·pill 4개·하드코딩 0). 이 사이클부터 **정적 export(`out/`) 없이 `.next` 산출**로 검증(Vercel 배포 전환, `CLAUDE.md` 구버전 확인됨). Branch `main`, 커밋 미완료(archive 후 사용자 요청 시) |
