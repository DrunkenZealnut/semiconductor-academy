...
| [mainpage-restructure](./mainpage-restructure/) | 2026-07-20 | 100% | 0 | 1 세션 | 홈 IA·동선 재배치 — 자료원이 2026-07-14 `homepage-learning-hub` 기준 4개에서 **11개**(교과서 8종 포함, 이후 menu-restructure 사이클에서 12개로 추가 성장)로 성장했으나 홈이 못 따라간 노후화 해소. `PlatformHero` 하드코딩 "4개 자료원"을 `SOURCES.length` 파생값으로 교정, **`SourcePicker`+`LearningPathSection`을 `PerspectiveCatalog` 1개로 통합**해 accent축(원리·위험·안전·직무) 군집화 — 관점당 자료원 1개 제약 해소로 **교과서 8종 전부 홈 노출**(이전엔 1종만). **`RoleQuickEntry` 신규**(목표 pill 4개 → 관점 군집 해시 앵커, 개인화·분기 페이지 없음). `#sources` 앵커(Header·Hero CTA 참조) 승계로 링크 무수정 — 동시 진행된 `menu-restructure`(헤더 드롭다운)와 겹침 없이 상보적으로 병합(홈=전체 카탈로그, 헤더=빠른 이동). sweep이 설계 범위 밖 동일 하드코딩 3곳(`seo.ts`·`Footer.tsx`·`page.tsx` 푸터)을 추가 적발해 함께 교정(FR-1 확장 적용, gap-detector가 "정당"으로 판정). gap-detector 독립 검증 **Match Rate 98%→100%**(G-1: 설계 문언 `<a>` vs 구현 `<Link>` — 기능 동일, 설계 문언을 코드에 정렬해 해소). 게이트 전부 통과(typecheck·lint·build exit 0, SSG 페이지 수 유지, 홈 렌더 실측 — 4클러스터 앵커·11자료원·8교과서·pill 4개·하드코딩 0). 이 사이클부터 **정적 export(`out/`) 없이 `.next` 산출**로 검증(Vercel 배포 전환, `CLAUDE.md` 구버전 확인됨). Branch `main`, origin/main(menu-restructure 등 14커밋) 병합 후 커밋 |
...
## 후속 백로그 (mainpage-restructure 파생)

- **커밋·PR**: 이번 사이클 변경분(신규 `PerspectiveCatalog.tsx`·`RoleQuickEntry.tsx` + `page.tsx`·`PlatformHero.tsx`·`seo.ts`·`Footer.tsx`·`types.ts`·`accent.ts` 수정 + `SourcePicker.tsx`·`LearningPathSection.tsx` 삭제)이 아직 커밋되지 않음 — 사용자 요청 시 진행
- **`Footer.tsx` 원본 크레딧 대표성 드리프트**: `Footer.tsx:35-36`가 11자료원 중 대표 4작(윤충식 외·OSHA·NCS·조우현 외)만 나열 — 수치 아닌 출처 표기라 FR-1(하드코딩 0) 위반은 아니나, 자료원이 더 늘면 대표성에서 벗어날 여지. 교과서 카테고리 통합 표기 등으로 별도 점검 권장
- **`PerspectiveCatalog` 교과서 그룹 시각 밀도**: 설계 §4는 교과서 8종에 컴팩트 카드(2열, `SourcePicker` 교과서 그룹 스타일)를 제안했으나, 구현은 전 그룹 균일 `SourceCard`로 통일 — 기능(FR-2)은 충족하나 원리 그룹만 카드 8장이라 스크롤이 상대적으로 김. 필요 시 컴팩트 변형 검토
- **CLAUDE.md 배포 섹션 구버전**: 이번 사이클에서 `output: 'export'`가 이미 제거돼 산출물이 `out/`이 아닌 `.next`임을 재확인(Vercel 배포 전환). 프로젝트 `CLAUDE.md`의 "GitHub Pages 배포" 서술이 실제와 어긋나므로 별도 사이클에서 갱신 권장([[deploy-is-vercel-not-pages]] 메모리 기존 확인 사항과 일치)
