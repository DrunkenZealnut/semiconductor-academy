...
- **커밋·PR**: 이번 사이클 변경분(`sources.ts`·`schoolTextMdx.tsx` 수정 + `src/content/sources/hs-assembly-inspection/` MDX 12 + `cross-link.json` 재생성)이 아직 커밋되지 않음 — 사용자 요청 시 진행
