...
2. **실습과제 원위치 재구성**(P4 방식 보정): 실습 전용 중단원이 없고 워크시트(실습목표/기기/재료/안전·유의/방법/평가)가 조작·유지보수 절 말미마다 붙는다 — 1차 스캔 11건(#4 ×2 · #5 ×2 · #8 ×1 · #9 ×1 · #11 ×3 · #12 ×2, **전수는 담당 배치가 확정·보고**) → 구현 확정 **13건**(#12 particle 실습이 2→4건으로 실측 — 원문 제목 이미지 손실 2건 발굴 포함). 실습과제는 **원문 위치 그대로 해당 모듈 안에** 실습과제 단위로 재구성(대표 상세+변형 압축), 같은 트랙 조작↔실습·유지보수 모듈은 SourceRef 상호 연결. 안전 유의사항 전건 보존(대표 개별+통합 warning Callout — 시리즈 확정 조항).
...
- 실습 커버리지: 실습과제 전수(**구현 확정 13건** — 1차 스캔 11건에서 particle +2 실측) 누락 0·안전 항목 누락 0·조작↔실습 상호 참조 정합.
