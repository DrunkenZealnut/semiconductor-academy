...
npm run verify:render -- --self-test   # 렌더 판정 대조군만 (Chrome만 필요 · 약 0.3초)
```
...
**`verify:render`도 자체검사를 갖는다** — 판정 대조군 **11건**이 `page.setContent()`로 만든 도해에
판정 함수를 그대로 먹인다. **서버도 자격 증명도 필요 없어** `--self-test`만 따로 돌릴 수 있다(약 0.3초).
