...
(자산 출처의 절대 경로는 라우트가 아니라 `public/` 자산이다 — 그쪽이 대부분이다). 종료 코드는 도해 관문과 같은 계약이다 — `1`은 **대상이 없다**,
`2`는 **검사기가 자기 범위를 주장할 수 없음**(파서 없음 · 대상 파일 0개 · **해석기 없는 동적 라우트** ·
`src/app`/데이터 못 읽음 · 자체검사 실패 · 대조군 수 불일치).
