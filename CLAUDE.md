...
npm run verify:render -- --self-test   # 렌더 대조군 36건만 (Chrome만 필요 · 약 23초)
npm run audit:wiring        # 렌더 관문 **배선 감사** — 관문이 아니라 관문을 재는 자 (약 6~7분)
```
...
만든 도해에 판정 함수를 그대로 먹인다) · **범위 10건**(순수 함수 `scopeStatic()`·`scopeViolations()`) · **처분 12건**(자식
프로세스를 임시 cwd에서 돌려 종료 코드와 사유를 본다 — **열둘 중 열하나는 가짜 도해 트리를 소유**해
...
현재 **배선 11/11 · 규칙 4/4(+가드 1)**. 가드는 *대조군이 잡을 수 없는* 훼손이다(부모가 모듈
