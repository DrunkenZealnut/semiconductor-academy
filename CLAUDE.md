...
npm run verify:render -- --self-test   # 렌더 대조군 33건만 (Chrome만 필요 · 약 15초)
npm run audit:wiring        # 렌더 관문 **배선 감사** — 관문이 아니라 관문을 재는 자 (약 2.5분)
```
...
**`verify:render`도 자체검사를 갖는다** — 대조군 **33건**이 세 무리다: **판정 14건**(`page.setContent()`로
만든 도해에 판정 함수를 그대로 먹인다) · **범위 10건**(순수 함수 `scopeStatic()`·`scopeViolations()`) · **처분 9건**(자식
프로세스를 임시 cwd에서 돌려 종료 코드와 사유를 본다 — **아홉 중 여덟은 가짜 도해 트리를 소유**해
저장소 상태와 무관하고, 하나만 실제 트리를 써서 *"유도 규칙이 실제 컴포넌트에서 도는가"* 를 지킨다). **판정을 순수 함수로 뽑으면 대조군이 싸지지만
그 함수를 부르는 자리(배선)가 무대조군이 된다** — 처분 무리가 그 자리를 덮고, `npm run audit:wiring`이
호출부 인자를 하나씩 훼손해 **몇 자리가 덮이는지 센다**(현재 **11/11**. 착수 시점은 6/11이었다).
**서버도 자격 증명도 필요 없어** `--self-test`만 따로 돌릴 수 있다(약 15초). 본 검사 앞에서 먼저 돌고 실패하면 본 검사를 실행하지 않는다(`exit 2`).
